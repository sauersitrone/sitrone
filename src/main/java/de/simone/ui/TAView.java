...
public abstract class TAView<E extends TAEntity> extends TVerticalLayout
        implements BeforeEnterObserver, ComponentEventListener<ClickEvent<Button>>, PropertyChangeListener {
    ...
    public Component getToolBarComponent(String id) {
        Optional<Component> optional = globalToolBar.stream().filter(c -> c.getId().orElse("null").equals(id))
                .findFirst();
        ...
        if (query != null) {
            ...
            if (!StringUtils.isBlank(query.partialWhere)) {
                ...
                if (min.contains(patt)) {
                    ...
                    String newSort = query.partialWhere.substring(min.length() - newSec[1].length(), min.length())
                            .trim();
    ...
    private void paginate(ClickEvent<Button> evt) {
        if (evt.getSource() == firstPageButton)
            listRequest.page = 0;
        if (evt.getSource() == prevPageButton)
            listRequest.page--;
        if (evt.getSource() == nextPageButton)
            listRequest.page++;
        if (evt.getSource() == lastPageButton)
            listRequest.page = listDataProvider.pages;
    ...
    public void onComponentEvent(ClickEvent<Button> event) {
        ...
        if (cmpId.startsWith(UIUtils.EXPORT_CSV_ACTION)) {
            /**
             * this code fragment do nothig on user autorized and only present error message
             * when not. see
    ...
    protected E getSelectedItem(ClickEvent<MenuItem> event) {
        ...
        E selectedItem = null;
        com.vaadin.flow.data.provider.ListDataProvider<E> dataProvider = ((com.vaadin.flow.data.provider.ListDataProvider<E>) grid
                .getDataProvider());
        for (E entity2 : dataProvider.getItems()) {
            if (id == entity2.id)
                selectedItem = entity2;
        }
        return selectedItem;
    }
