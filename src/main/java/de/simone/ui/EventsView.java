...
public class EventsView extends TAView<Event> {
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        SerializablePredicate<Event> filter = entity -> {
            String searchTerm = searchField.getValue().trim().toLowerCase();
            if (searchTerm.isEmpty())
                return true;

            boolean m1 = entity.title.toLowerCase().contains(searchTerm);
            boolean m2 = TranslationProvider.getString("event.type", entity.type)
                    .toLowerCase()
                    .contains(searchTerm);
            boolean m3 = TranslationProvider.getString("event.status", entity.status)
                    .toLowerCase()
                    .contains(searchTerm);
