...
public class Tamagotchi extends TAEntity {
  ...
  @Transient public long lastMillis;
  @Transient public long lastAction;
  ...
  /**
   * Making a character eat some food If the character overeats making it sadder If character was
   * hungry eating makes it happier There is also a 50/50 chance that a character gets a little
   * dirty while eating
   */
  public String eat() {
  ...
  /**
   * Getting character cleaner If the character is already clean enough then it maks the character
   * sad and drains energy If the character needs a little shower then adding an appropriate value
  ...
  /**
   * Getting a character to sleep. Setting sleeping field to false and saving a time when a
  ...
  /**
   * Waking up a character from a sleep, and making character's go up according to the amount of
   * minutes that elapsed from character going to sleep. Setting a sleeping field to false
   */
  ...
  /**
   * Playing with a character which makes it happier, hungrier, dirtier, healthier and lighter. WHAT
   * A COMBO! When the character doesn't have enough energy for that activity it loses some health
  ...
  /**
   * Character level is going up. Only when the experience level is above 100 Otherwise we do
   * nothing
   */
  ...
  public void timePassed() {
    if (System.currentTimeMillis() - lastMillis < 60 * 1000) return;
    ...
    // check inactivitiy
    long secs = Duration.parse(INACTIVE).getSeconds() * 1000;
    if (System.currentTimeMillis() - lastAction > secs) Tamagotchi.getEntityManager().merge(this);
  }
